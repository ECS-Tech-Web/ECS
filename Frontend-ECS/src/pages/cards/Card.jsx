...
export default function Card({ title, description, imageUrl, instagram, facebook, linkedin, theme = "cyan" }) {
  ...
  const style = colorMap[theme] || colorMap.cyan; // Safely fall back to standard cyan

  return (
    <div className={`group relative h-[42vh] w-[32vh] flex flex-col justify-between p-5 backdrop-blur-md rounded-[24px] border overflow-hidden transition-all duration-500 hover:-translate-y-2 ${style.cardBg} ${style.border} ${style.shadow}`}>
      ...
      {/* 2. IMAGE CONTAINER */}
      <div className="relative w-full h-[58%] flex justify-center items-center overflow-hidden rounded-xl bg-black/30 border border-slate-800/60 group-hover:border-slate-700 transition-colors duration-500">
        <img
          src={imageUrl}
          alt={title}
      ...
      {/* 3. INFO PANEL */}
      <div className="relative z-10 flex flex-col justify-between flex-grow mt-4 text-center">
        
        {/* Text Block */}
        <div className="space-y-1">
          <h2 className={`text-xl font-bold tracking-wide font-sans group-hover:text-white transition-colors duration-300 ${style.textTitle}`}>
            {title}
          </h2>
          <p className={`text-sm text-slate-300 font-medium tracking-wide transition-colors duration-300 ${style.textSub}`}>
            {description}
          </p>
        </div>

        {/* 4. PERFECTED HOVER SOCIAL TRAYS */}
        <div className="flex items-center justify-center gap-3.5 pt-3 opacity-0 translate-y-3 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-out">
          {instagram && (
            <a 
              href={instagram} 
              target="_blank" 
